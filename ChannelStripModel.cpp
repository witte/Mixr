...
bool ChannelStripModel::setData(const QModelIndex &index, const QVariant &value, int role = Qt::EditRole) //Q_DECL_OVERRID
{
    if ( (index.row() >= 0) && (index.row() < m_ChannelStrips.length()) ) {
        ...
        switch (role) {
            ...
            case PanRole:       item->setPan(value.toFloat()); break;
            case VolumeRole:    item->setVolume(value.toFloat()); break;
            case TempMuteRole:  item->setTempMute(value.toBool()); break;
            case MuteRole:      item->setMute(value.toBool()); break;
            case SoloRole:      item->setSolo(value.toBool()); break;
...
bool ChannelStripModel::setVolume(int row, const float &value) { return setData(createIndex(row, 0), value, VolumeRole); }
